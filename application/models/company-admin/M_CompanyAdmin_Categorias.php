...
class M_CompanyAdmin_Categorias extends CI_Model {
	...
	public function getCategoryByNivel($category_data) {
		$this->db->where('Categoria_Productos.id_empresa', $category_data["id_empresa"]);
		$this->db->where('Categoria_Productos.nivel_categoria', $category_data["nivel_categoria"]);
		$this->db->where('Categoria_Productos.estado', '1');
		$query = $this->db->get('Categoria_Productos');
	...
	public function insertIdImageCategory($data) {
		$this->db->where('Categoria_Productos.id_categoria', $data["id_categoria"]);
		if ($this->db->update('Categoria_Productos', array('Categoria_Productos.id_imagen_categoria'=> $data["id_imagen_categoria"])))	{
			return TRUE;
		}
		return FALSE;
	}
