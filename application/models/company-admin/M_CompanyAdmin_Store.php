...
class M_CompanyAdmin_Store extends CI_Model {
    ...
    public function updateStore($dataStore) {
        $data = array(
            "nombre_tienda"     => $dataStore["nombre_tienda"],
            "nro_celular"       => $dataStore["nro_celular"],
            "nro_telefono"      => $dataStore["nro_telefono"],
            "pais"              => $dataStore["pais"],
            "region"		    => $dataStore["region"],
            "ciudad"		    => $dataStore["ciudad"],
            "direccion"		    => $dataStore["direccion"],
            "gps_latitud"       => $dataStore["gps_latitud"],
            "gps_longitud"      => $dataStore["gps_longitud"]
        );
        $this->db->where('id_tienda', $dataStore["id_tienda"]);
        if ($this->db->update('Tienda', $data)) {
            return TRUE;
        }
